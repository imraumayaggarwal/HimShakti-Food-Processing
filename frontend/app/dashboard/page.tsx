    ...
    <div className="bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 rounded-3xl p-6 flex flex-col gap-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
      ...
      <div className="flex items-center justify-between pt-2 border-t border-black/5 dark:border-white/5 mt-auto">
        <div className="flex gap-4">
          {product.price != null && (
            <span className="text-sm font-semibold text-[#356C4C]">₹{product.price}</span>
          )}
          {product.stock != null && (
            <span className="text-sm text-black/40 dark:text-white/40">{product.stock} in stock</span>
          )}
        </div>
        <button
...
function AddProductModal({
  ...
  open: boolean;
  onClose: () => void;
  onAdd: (product: Omit<Product, "id" | "created_at">) => Promise<void>;
}) {
  const [form, setForm] = useState({
    name: "", ingredients: "", weight: "", category: "other",
    features: "", price: "", stock: "", description: "",
  });
      ...
      <div className="relative bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 rounded-3xl p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="space-y-4">
          ...
          <Input
            ...
            placeholder="Gluten-free, No preservatives, Handmade"
            value={form.features}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Leave blank — generate it with AI on the Generate page"
              rows={3}
        ...
        <div className="flex gap-3 mt-6">
          <button
            onClick={onClose}
            className="flex-1 py-3 rounded-full border border-black/10 dark:border-white/10 text-black dark:text-white text-sm hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={handleSubmit}
            disabled={saving || !form.name || !form.ingredients || !form.weight}
...
export default function DashboardPage() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      setError("Couldn't connect to the backend. Make sure the server is running on port 8000.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="max-w-6xl mx-auto px-6 pt-36 pb-24">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
          <div>
            ...
            <p className="text-black/50 dark:text-white/50 mt-1 text-sm">Manage your products and inventory.</p>
          </div>
